...
def depthFirstSearch(problem):
    ...
    '''
    we need to intialize our stack with where we start and then for
    now an empty array we can use to track the path we took to get there
    '''
    ...
    while not frontier.isEmpty():
        ...
        if current_state not in visited_states:
            ...
            '''
            pacboi makes an action so we get back the succesor state itself, the action
            taken i.e 'North' 'South' and cost of action which i dont think we care for so _
            '''
...
def breadthFirstSearch(problem):
    ...
    while not frontier.isEmpty():
        ...
        # exploration, look at succ states and if its not in our visited states push into frontier 
        for successor, action, _ in problem.successorStates(current_state):
            if successor not in visited_states:
                frontier.push((successor, path + [action]))
...
def uniformCostSearch(problem):
    ...
    '''
    Okay now we know that our frontier is going to act as a Priority Queue in which we keep 
    track of cost
    '''
    ...
    while not frontier.isEmpty():
        ...
        # state visited with cost <= to cur_cost no need to vis again better path found
...
def aStarSearch(problem, heuristic):
    ...
    ''' 
    okay given that this is an A* search we need to list (closed, open) in order to 
    keep tracks of evaluated nodes and nodes to explore given that we can make our
    fronteir our open list that keeps tracks of nodes to explore and evaluated states
    as the closed list
    '''
    frontier = PriorityQueue()
    eval_states = set()
    ...
    while not frontier.isEmpty():
        ...
        for successor, action, step_cost in problem.successorStates(current_state):
            ...
            get the cost of current state and the step cost to the neighbor, as well as the
            huersitic value of the measurement and add them
            '''
            successor_cost = cost + step_cost
            successor_h = heuristic(successor, problem)
            successor_eval = successor_cost + successor_h
            '''
            if a successor, or neighboring state has a cheaper cost, or has not been
            discovered yet push that into our PQ
            '''
